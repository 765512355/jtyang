
# Jintao Yang (杨锦涛)

**Associate Professor, College of Economics & Management**  
**China Agricultural University**

📧 Email: yangjt@lreis.ac.cn  
📖 [Google Scholar Profile](https://scholar.google.com/citations?user=O6IYuLgAAAAJ&hl=en )

---

## About Me
  
I am an Associate Professor at China Agricultural University, working at the intersection of GIS, environmental science, agricultural economics, and sustainability analytics.

---

## My research centers on two major directions:

### 1. Spatiotemporal process modeling of agricultural–environmental systems

I develop spatially explicit models to understand how agricultural production, pollutant migration, ecological processes, and human behaviors evolve across space and time. I integrate GIS, remote sensing, spatial statistics, and system simulation to uncover heterogeneity, dynamics, and feedbacks within coupled human–environment systems. 

### 2. Optimization and governance of sustainable agri-food systems

I design optimization and policy frameworks to achieve low-carbon development, heavy-metal pollution governance, and biodiversity protection. My work quantifies environmental footprints embedded in interprovincial food circulation and proposes implementable solutions that support China’s dual-carbon goals, food safety strategies, and ecological compensation mechanisms.

Overall, my research aims to provide data-driven, model-supported, and polic
---

## Methods & Tools

- **Spatial modeling**: GIS, Remote Sensing, GeoDetector, spatial autocorrelation and regression  
- **Optimization**: Linear programming, multi-objective optimization, heuristic algorithms (GA, ACO)  
- **Machine learning and forecasting**: Random Forest, LSTM / Seq2Seq for time series  
- **Ecosystem & risk models**: InVEST, DNDC, exposure assessment frameworks  

---

## Selected Publications

- Yang J. et al. (2025). *Internalizing the external costs to achieve environmental and economic goals: A case study of rice production in China.* Food Policy.  
- Yang J. et al. (2024). *Cross-provincial inpatient mobility patterns and their determinants in China.* BMC Health Services Research.  
- Yang J. et al. (2022). *Modeling the spatial relationship between rice cadmium and soil properties at a regional scale considering confounding effects and spatial heterogeneity.* Chemosphere.  

A more complete list is available on my [Google Scholar Profile](https://scholar.google.com/citations?user=O6IYuLgAAAAJ&hl=en ).

---

## Teaching

- Low-carbon Economic Management  
- Spatial Modeling and Optimization for Agri-Food Systems  
- GIS & Big Data Applications in Food Safety and Environmental Governance  

I am happy to work with students interested in **GIS + food systems + environmental policy**.

---

## Collaboration

I am open to collaborations on topics such as:

- Low-carbon transformation of rice and staple food systems  
- Heavy metal risk governance in agricultural regions  
- Biodiversity, ecosystem services, and agricultural policy design  

Feel free to reach out by email.
